...
class LocalEnergyStats(Callback):
    ...
    def add_energy_stats_to_logs(self, logs, generator, prefix=""):
        logs['%senergy/energy' % prefix] = numpy.real(generator.energy_observable.current_energy)
        logs['%senergy/local_energy_variance' % prefix] = numpy.real(generator.energy_observable.current_local_energy_variance)
        if self.true_ground_state_energy is not None:
            logs['%senergy/relative_error' % prefix] = (self.true_ground_state_energy - numpy.real(generator.energy_observable.current_energy)) / self.true_ground_state_energy
