...
class RBM(Machine):
    def __init__(self, keras_input_layer, alpha=1.0, **kwargs):
        super(RBM, self).__init__(keras_input_layer, **kwargs)
        x = ToComplex64()(self.keras_input_layer)
        x = Flatten()(x)
        self._lnthetas = ComplexDense(units=alpha, activation=lncosh, multiply_units_by_input_dim=True)(x)
...
class RBMSym(Machine):
    def __init__(self, keras_input_layer, alpha=1.0, **kwargs):
        ...
        x = ToComplex64()(self.keras_input_layer)
        self._lnthetas = TranslationInvariantComplexDense(units=alpha, activation=lncosh)(x)
        x = Flatten()(x)
