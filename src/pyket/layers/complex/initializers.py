...
class ComplexIndependentFilters(Initializer):
    ...
    # that are independent as much as possible from each other
    # while respecting either the He or the Glorot criterion.
    def __init__(self, criterion='glorot', seed=None, reverse_channels=False):
        self.criterion = criterion
        self.rng = RandomState(seed)
        self.reverse_channels = reverse_channels

    def __call__(self, shape, dtype=None, partition_info=None):
        kernel_size, input_dim, nb_filters = shape[0:2], shape[2], shape[3]
        num_rows = nb_filters * input_dim
        num_cols = np.prod(kernel_size)

        flat_shape = (int(num_rows), int(num_cols))
        r = self.rng.uniform(size=flat_shape)
        i = self.rng.uniform(size=flat_shape)
        z = r + 1j * i
        ...
        if self.reverse_channels:
            ...
            fan_in, fan_out = _compute_fans(tuple([shape[0], shape[1], shape[3], shape[2]]))
        ...
        weight_real = np.reshape(weight_real, shape)
        weight_imag = np.reshape(weight_imag, shape)
...
class ComplexInit(Initializer):
    ...
    def __call__(self, shape, dtype=None, partition_info=None):
        ...
        modulus = self.rng.rayleigh(scale=s, size=shape)
        phase = self.rng.uniform(low=-np.pi, high=np.pi, size=shape)
...
complex_independent_filters = ComplexIndependentFilters
