...
callbacks = default_wave_function_stats_callbacks_factory(monte_carlo_generator, log_in_batch_or_epoch=False,
                                                          true_ground_state_energy=true_ground_state_energy) + [tensorboard, TerminateOnNaN(), warly_stopping]
model.fit_generator(monte_carlo_generator.to_generator(), steps_per_epoch=steps_per_epoch, epochs=60, callbacks=callbacks,
...
if warly_stopping.stopped_epoch is not None:
    ...
    sys.exit()
print('incresing batchsize to 10000')

sampler = FastAutoregressiveSampler(conditional_log_probs_model, batch_size * 8)
monte_carlo_generator.set_sampler(sampler)
model.fit_generator(monte_carlo_generator.to_generator(), steps_per_epoch=steps_per_epoch, epochs=80, callbacks=callbacks,
                    max_queue_size=0, workers=0)
