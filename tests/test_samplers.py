...
@pytest.mark.parametrize('sampler_class, one_or_two_dim, machine_class, machine_args', [
    (AutoregressiveSampler, True, SimpleConvNetAutoregressive1D,
     {'depth': 5, 'num_of_channels': 16, 'weights_normalization': False}),
    (AutoregressiveSampler, False, ConvNetAutoregressive2D,
     {'depth': 2, 'num_of_channels': 16, 'weights_normalization': False}),
    (FastAutoregressiveSampler, True, SimpleConvNetAutoregressive1D,
     {'depth': 5, 'num_of_channels': 16, 'weights_normalization': False}),
    (FastAutoregressiveSampler, False, ConvNetAutoregressive2D,
     {'depth': 2, 'num_of_channels': 16, 'weights_normalization': False}),
    (MetropolisHastingsLocal, True, SimpleConvNetAutoregressive1D,
     {'depth': 5, 'num_of_channels': 16, 'weights_normalization': False}),
    (MetropolisHastingsLocal, False, ConvNetAutoregressive2D,
     {'depth': 2, 'num_of_channels': 16, 'weights_normalization': False}),
])
def test_sampler_by_l1(sampler_class, one_or_two_dim, machine_class, machine_args):
    # this test based on https://arxiv.org/pdf/1308.3946.pdf
    if one_or_two_dim:
        machine_input = Input(shape=(10,), dtype='int8')
    else:
        machine_input = Input(shape=(4, 3), dtype='int8')
