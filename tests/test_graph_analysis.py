...
def create_test_case(idx):
    return [(Concatenate(), [Input((6, 6, 5)), Input((6, 6, 5))], 32),
...
def test_apply_layer_for_single_spatial_location(idx):
    layer, input_layer, batch_size = create_test_case(idx)
    print(layer)
...
def get_layer_output_with_topology_manager(has_multiple_inputs, input_layer, layer):
    ...
    for spatial_location in itertools.product(*[range(dim_shape) for dim_shape in
                                                layer.get_output_shape_at(0)[1:-1]]):
        dependencies_values = []
